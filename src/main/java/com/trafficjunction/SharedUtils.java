...
public class SharedUtils {
    public static boolean verifyLanes(String[] lanetypes, boolean oneLightMode) {
        ...
        if (oneLightMode) {
            enum stage {
                leftAllowed, forwardAllowed, rightAllowed
            }
            ...
            for (int i = 0; i < lanetypes.length; i++) {
                if (currentStage == stage.leftAllowed && !lanetypes[i].equals("L")) {
                    if (lanetypes[i].equals("LF") || lanetypes[i].equals("F"))
                        currentStage = stage.forwardAllowed;
                    else
                        currentStage = stage.rightAllowed;
                } else if (currentStage == stage.forwardAllowed && !lanetypes[i].equals("F")) {
                    if (lanetypes[i].equals("L") || lanetypes[i].equals("LF"))
                        return false;
                    else
                        currentStage = stage.rightAllowed;
